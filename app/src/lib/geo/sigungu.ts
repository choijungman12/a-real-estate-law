/**
 * 한국 17개 시·도 + 주요 시·군·구 코드 매핑
 * 국토부 실거래가 API의 LAWD_CD (법정동코드 5자리) 사용
 *
 * (전체 시군구 250+ 중 부동산 거래가 활발한 곳 위주 약 120개 수록.
 * 추후 행정안전부 법정동 코드 전체 데이터로 확장 가능)
 */

export type SigunguEntry = { code: string; name: string };

export const SIDO: { code: string; name: string; sigungu: SigunguEntry[] }[] = [
  {
    code: '11',
    name: '서울특별시',
    sigungu: [
  ...
  { code: '36', name: '세종특별자치시', sigungu: [{ code: '36110', name: '세종시' }] },
  {
    code: '41',
    name: '경기도',
    sigungu: [
...
export function findSigungu(code: string): { sido: string; name: string } | undefined {
  for (const s of SIDO) {
    const found = s.sigungu.find((x) => x.code === code);
    if (found) return { sido: s.name, name: found.name };
  }
  return undefined;
}
